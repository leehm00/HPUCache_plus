...
#define MAX_LRU_TRACK_NUM 10//define the hottest slot tracklist length 
...
#define CLUSTERMSG_TYPE_LRU 12          /* lrulist sync types */
...
typedef struct clusterState {
    ...
    /* homie: Number of the requests for each slots */
    ...
    bool sync_time;//if we forward the write commands immediately, we set it false(meaning we don't need to wait to sync), otherwise true
    bool forward_time;//a bool to decide how we forward the write command, avg or skew. use find to see detailed useage
    void *lru_hot;//a list to track the hottest slot
    // int lrulist[MAX_LRU_TRACK_NUM][2];//list to track hotslot
} clusterState;
...
typedef struct {
    ...
    char nodename[CLUSTER_NAMELEN]; /* my node name*/
} clusterMsgDataHotGroup;

typedef struct {
    //used to update the local lru information of each node
    uint32_t slotname[MAX_LRU_TRACK_NUM];
    uint32_t opnum[MAX_LRU_TRACK_NUM]; 
    char nodename[CLUSTER_NAMELEN]; /* my node name*/
} clusterMsgLru;
typedef struct {
