...
# RECORDCOUNT=10000000 # 100M
RECORDCOUNT=100000000 # 100M
OPERCOUNT=1000000 # for 50 client epach have 0.5M total 20M * 4 = 80M
...
runWorkload() {
    ...
    for hg in ${HG_NUM}; do
        ...
        sleep 5
        opercounts=$(keysizetokeynum $keysize)
        loadData ${RECORDCOUNT} ${ENTRY_IP} ${ENTRY_PORT} ${keysize}

        LAST_COPY_NUM=0
        for WLT in ${WORKLOADTYPES}; do
        echo "running ${hg} ${WLT} no"
            # sleep 20
            # doCopy ${c_size} ${WLT}
            # sleep 20
            ...
            multiClientRun ${WLT} ${OPERCOUNT} \
                           ...
                           ${ENTRY_IP} ${ENTRY_PORT} ${is_skew} ${keysize}
            stopMonitor
            # getRedisStat $WLT $DISTRIBUTION
            moveData ${keysize} ${WLT}
            cleanLog
            sleep 10
        done
        stopCluster ${MACH}
...
# zipfians="03"
for keysize in ${fieldlens}; do
    LOG_NAME="${keysize}"
    LOG_DIR="/home/homie/share/redis/log/normal/${LOG_NAME}"
    HG_DIR="/home/homie/share/redis/utils/HG_no"
    runWorkload yes $keysize
    # opercounts=$(keysizetokeynum $keysize)
    # echo $opercounts
    # echo "running yes ${s}"
done
